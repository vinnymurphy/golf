def calculate_handicap(player_user):
    ...
    rounds = Round.objects.filter(player=player_user).order_by('-date')[:20]
    count = rounds.count()
    ...
    if count >= 20: num_to_use = 8
    elif count >= 15: num_to_use = 6
    elif count >= 10: num_to_use = 3
    else: num_to_use = 1 # Minimum requirement
